    ...
    def next(self):
        if self.current < 20:
            ...
            for i in range(20):
                ...
                self.canvas.create_text(10 + i * barWidth + 5,
            ...
            self.canvas.create_rectangle(10 + self.current * barWidth, height - (height - 10) * self.counts[self.current ] / self.maxCount,
                                         10 + (self.current  + 1) * barWidth, height - 10, fill='red',tags='histogram')
            self.current += 1
    def reGen(self):
        self.canvas.delete('histogram')
        ...
        for i in range(20):
            self.canvas.create_rectangle(10+i*barWidth, height-(height-10)*self.counts[i]/self.maxCount,
                                         10+(i+1)*barWidth, height-10, tags='histogram')
            self.canvas.create_text(10+i*barWidth+5,height-(height-10)*self.counts[i]/self.maxCount-5,
                                    text = str(self.counts[i]),tags='histogram')

    def __init__(self):
        window = Tk()
        window.title('선택 정렬 애니메이션')
        self.canvas = Canvas(window, width=width,heigh=height,bg = 'white')
        self.canvas.pack()
        frame = Frame(window)
        frame.pack()
