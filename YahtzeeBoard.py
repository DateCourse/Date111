...
class YahtzeeBoard:
    ...
    def playerNames(self):
        ...
        for i in range(1, self.numPlayers+1):
            self.players.append(Player2(str(self.entry[i].get())))
    ...
    def nextTurn(self):
        ...
        self.bottomLabel.configure(text=self.players[self.player].toString() + "차례: Roll Dice")
        ...
        if self.round == 13:
            ...
            for i in range(self.numPlayers):
                if tmp < self.players[i].getTotalScore() + self.players[i].bonus:
                    ...
                    winner = i
            tkinter.messagebox.showinfo("결과","승자 : "+self.players[winner].toString()+" 임다.\n\t"+str(tmp)+"점")
    ...
    def initInterface(self):
        ...
        self.bottomLabel = Label(self.window, text=self.players[self.player].toString()+"차례: Roll Dice", \
