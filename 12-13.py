...
class MainGUI:
    def check(self):
        for i in range(6):
            for j in range(4):
                ...
                if player != ' ' and player == self.matrix[i][j + 1]['text'] and player == self.matrix[i][j + 2]['text'] and player == self.self.matrix[i][
                    j + 3]['text']:
                    return player
    ...
    def refresh(self):
        for i in range(3):
            for j in range(3):
                self.self.matrix[i][j]['image'] = self.imageE
                self.self.matrix[i][j]['text'] = ' '
        self.done = False
        self.explain.set('플레이어 X 차례')
        self.turn = True
    def __init__(self):
